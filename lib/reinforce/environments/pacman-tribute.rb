...
module Reinforce
  module Environments
    ...
    class Pacman
      ...
      # Each vector  will represent the item type (agent_position, item) and the position
      # we use an integer to represent the item type: 0 -> agent_position,
      # 1 -> goal, 2 -> item
      ...
      # item: [2, 6, 7, 0] -> item is at position 6,7. here 0 means that 
      # the item is not picked up, 1 means that the item is picked up
      def initialize(size, start, object_num = 5)
        @size = size
        @start = start
